...
class MathParserTest {
    ...
    fun testBasicOperations() {
        val fn = MathParser.parse("2 + 2")
        assertEquals(4.0, fn(0.0), 0.001)
    }

    @Test
    fun testVariable() {
        val fn = MathParser.parse("x^2")
    ...
    @Test
    fun testFunctions() {
        val fn = MathParser.parse("sin(0)")
        assertEquals(0.0, fn(0.0), 0.001)
        
        val fn2 = MathParser.parse("cos(0)")
    ...
    fun testComplicated() {
        // sin(x) + 1 at x=0 => 1
        val fn = MathParser.parse("sin(x) + 1")
    ...
    fun testImplicitMultiplication() {
        // 2x at x=3 => 6
        val fn = MathParser.parse("2x")
        assertEquals(6.0, fn(3.0), 0.001)

        // 3sin(x) at x=pi/2 => 3
        val fn2 = MathParser.parse("3sin(x)")
